...
Set `ADMIN_TOKEN` env var (default: `ec-admin-cp04-2026`).  
Must match `ADMIN_TOKEN` constant in `index.html`.

> [!NOTE]
> `GET /api/sessions` (unauthenticated) was removed in v4 — it was leaking all client PII.
> Use `GET /api/sessions/admin` with the `X-Admin-Token` header instead.
